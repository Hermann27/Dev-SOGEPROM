...
Public Class ModeleReglement
    Private Sub AfficheSchemasConso()
        Try
            ...
            OleAdaptaterschema = New OleDbDataAdapter("select * from MODELEREGLEMENT", OleConnenection)
            ...
            For i = 0 To OledatableSchema.Rows.Count - 1
                DataListeIntegrer.Rows(i).Cells("Catego1").Value = OledatableSchema.Rows(i).Item("Code")
                DataListeIntegrer.Rows(i).Cells("Intitu1").Value = OledatableSchema.Rows(i).Item("Intitule")
            Next i
        Catch ex As Exception
            MsgBox("Message Systeme: " & ex.Message, MsgBoxStyle.Information, "Modele de Reglement")
        End Try
    End Sub
    Private Sub Delete_DataListeSch()
        ...
        For i = 0 To DataListeIntegrer.RowCount - 1
            If DataListeIntegrer.Rows(i).Cells("Supprimer").Value = True Then
                OleAdaptaterDelete = New OleDbDataAdapter("select * from MODELEREGLEMENT where Code='" & DataListeIntegrer.Rows(i).Cells("Catego1").Value & "'", OleConnenection)
                ...
                If OledatableDelete.Rows.Count <> 0 Then
                    DelFile = "Delete From MODELEREGLEMENT where Code='" & DataListeIntegrer.Rows(i).Cells("Catego1").Value & "'"
        ...
        Try
            ...
            If DataListeSchema.RowCount >= 1 Then
                For n = 0 To DataListeSchema.RowCount - 1
                    OleAdaptaterEnreg = New OleDbDataAdapter("select * From MODELEREGLEMENT WHERE Code='" & DataListeSchema.Rows(n).Cells("Catego").Value & "'", OleConnenection)
                    ...
                    If OledatableEnreg.Rows.Count <> 0 Then
                        MsgBox("Ce Modele Existe déja!", MsgBoxStyle.Information, "Creation Modele de Reglement Comptable")
                    Else
                        If Trim(DataListeSchema.Rows(n).Cells("Catego").Value) <> "" Then
                            Insertion = "Insert Into MODELEREGLEMENT (Code,Intitule) VALUES ('" & DataListeSchema.Rows(n).Cells("Catego").Value & "','" & DataListeSchema.Rows(n).Cells("Intitu").Value & "')"
                ...
                Next n
                If Insert = True Then
                    MsgBox("Insertion Reussie", MsgBoxStyle.Information, "Parametrage des Modeles de Reglement")
                    DataListeSchema.Rows.Clear()
                End If
            End If
        Catch ex As Exception
            MsgBox("Message Systeme :" & ex.Message, MsgBoxStyle.Information, "Modeles de Reglement")
        End Try
    End Sub
    ...
    Private Sub ModeleReglement_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Dim boll As Boolean
        boll = Connected()
        AfficheSchemasConso()
