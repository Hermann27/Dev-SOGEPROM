...
Public Class Frm_Client
    Private Sub AfficheSchemasConso()
        Try
            ...
            OleAdaptaterschema = New OleDbDataAdapter("select * from CLIENT", OleConnenection)
    ...
    Private Sub Delete_DataListeSch()
        ...
        For i = 0 To DataListeIntegrer.RowCount - 1
            If DataListeIntegrer.Rows(i).Cells("Supprimer").Value = True Then
                OleAdaptaterDelete = New OleDbDataAdapter("select * from CLIENT where Plant='" & DataListeIntegrer.Rows(i).Cells("Catego1").Value & "'", OleConnenection)
                ...
                If OledatableDelete.Rows.Count <> 0 Then
                    DelFile = "Delete From CLIENT where Plant='" & DataListeIntegrer.Rows(i).Cells("Catego1").Value & "'"
        ...
        Try
            ...
            If DataListeSchema.RowCount >= 1 Then
                For n = 0 To DataListeSchema.RowCount - 1
                    OleAdaptaterEnreg = New OleDbDataAdapter("select * From CLIENT WHERE Plant='" & DataListeSchema.Rows(n).Cells("Catego").Value & "'", OleConnenection)
                    ...
                    Else
                        If Trim(DataListeSchema.Rows(n).Cells("Catego").Value) <> "" Then
                            Insertion = "Insert Into CLIENT (Plant,Collectif,Caption) VALUES ('" & DataListeSchema.Rows(n).Cells("Catego").Value & "','" & DataListeSchema.Rows(n).Cells("Intitu").Value & "','" & DataListeSchema.Rows(n).Cells("Caption").Value & "')"
    ...
    Private Sub Frm_Client_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Dim boll As Boolean
        boll = Connected()
        AfficheSchemasConso()
