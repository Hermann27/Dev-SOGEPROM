...
Imports System.IO
Public Class ClientSchematintegrer
    Private Sub ClientSchematintegrer_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub AfficheFormat()
        ...
        Try
            OleAdaptater = New OleDbDataAdapter("select * from FORMATIERS", OleConnenection)
    ...
    Private Sub AfficheSchemasIntegrer()
        ...
        Try
            OleAdaptaterschema = New OleDbDataAdapter("select * from PARACLIENTS", OleConnenection)
        ...
        Try
            For i = 0 To DataListeIntegrer.RowCount - 1
                If DataListeIntegrer.Rows(i).Cells("Supprimer").Value = True Then
                    OleAdaptaterDelete = New OleDbDataAdapter("select * from PARACLIENTS where Format='" & DataListeIntegrer.Rows(i).Cells("CheminForma").Value & "' and Fichier='" & DataListeIntegrer.Rows(i).Cells("CheminRepexpor").Value & "'", OleConnenection)
                    ...
                    If OledatableDelete.Rows.Count <> 0 Then
                        DelFile = "Delete From PARACLIENTS where Format='" & DataListeIntegrer.Rows(i).Cells("CheminForma").Value & "' and Fichier='" & DataListeIntegrer.Rows(i).Cells("CheminRepexpor").Value & "'"
        ...
        Try
            If DataListeSchema.RowCount >= 1 Then
                For n = 0 To DataListeSchema.RowCount - 1
                    OleAdaptaterEnreg = New OleDbDataAdapter("select * From PARACLIENTS WHERE Format='" & DataListeSchema.Rows(n).Cells("Chemin").Value & "' and Fichier='" & DataListeSchema.Rows(n).Cells("CheminExport").Value & "'", OleConnenection)
                    ...
                    Else
                        If Trim(DataListeSchema.Rows(n).Cells("Chemin").Value) <> "" And Trim(DataListeSchema.Rows(n).Cells("CheminExport").Value) <> "" Then
                            Insertion = "Insert Into PARACLIENTS (Format,Fichier,Nom_Format,Nom_Fichier) VALUES ('" & DataListeSchema.Rows(n).Cells("Chemin").Value & "','" & DataListeSchema.Rows(n).Cells("CheminExport").Value & "','" & DataListeSchema.Rows(n).Cells("NomFormat").Value & "','" & DataListeSchema.Rows(n).Cells("DossierExport").Value & "')"
    ...
    Private Sub DataListeSchema_CellEndEdit(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DataListeSchema.CellEndEdit
        ...
        Try
            If e.RowIndex >= 0 Then
                OleAdaptaterRecher = New OleDbDataAdapter("select * from FORMATIERS where Nom_Format='" & DataListeSchema.Rows(e.RowIndex).Cells("NomFormat").Value & "'", OleConnenection)
