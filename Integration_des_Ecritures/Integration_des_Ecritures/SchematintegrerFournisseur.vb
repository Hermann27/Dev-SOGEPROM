...
Imports System.IO
Public Class SchematintegrerFournisseur
    Private Sub SchematintegrerFournisseur_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Try
            For i = 0 To DataListeIntegrer.RowCount - 1
                If DataListeIntegrer.Rows(i).Cells("Supprimer").Value = True Then
                    OleAdaptaterDelete = New OleDbDataAdapter("select * from PARATIERS where Format='" & DataListeIntegrer.Rows(i).Cells("CheminForma").Value & "' and Fichier='" & DataListeIntegrer.Rows(i).Cells("CheminRepexpor").Value & "'", OleConnenection)
                    ...
                    If OledatableDelete.Rows.Count <> 0 Then
                        DelFile = "Delete From PARATIERS where Format='" & DataListeIntegrer.Rows(i).Cells("CheminForma").Value & "' and Fichier='" & DataListeIntegrer.Rows(i).Cells("CheminRepexpor").Value & "'"
        ...
        Try
            If DataListeSchema.RowCount >= 0 Then
                For n = 0 To DataListeSchema.RowCount - 1
                    OleAdaptaterEnreg = New OleDbDataAdapter("select * From PARATIERS WHERE Format='" & DataListeSchema.Rows(n).Cells("Chemin").Value & "' and Fichier='" & DataListeSchema.Rows(n).Cells("CheminExport").Value & "'", OleConnenection)
                    ...
                    Else
                        If Trim(DataListeSchema.Rows(n).Cells("Chemin").Value) <> "" And Trim(DataListeSchema.Rows(n).Cells("CheminExport").Value) <> "" Then
                            Insertion = "Insert Into PARATIERS (Format,Fichier,Nom_Format,Nom_Fichier) VALUES ('" & DataListeSchema.Rows(n).Cells("Chemin").Value & "','" & DataListeSchema.Rows(n).Cells("CheminExport").Value & "','" & DataListeSchema.Rows(n).Cells("NomFormat").Value & "','" & DataListeSchema.Rows(n).Cells("DossierExport").Value & "')"
