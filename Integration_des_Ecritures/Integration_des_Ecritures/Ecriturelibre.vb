...
Public Class Ecriturelibre
    ...
    Private Sub Ecriturelibre_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub UpdateParametre()
        ...
        Try
            If Trim(DateDebut.Text) <> "" And Trim(Periode.Text) <> "" And Trim(DateFin.Text) <> "" Then
                OleUpdatAdaptater = New OleDbDataAdapter("select * From COECRITURE where Libelle='" & Trim(Periode.Text) & "' Or Colonne='" & Trim(DateDebut.Text) & "'", OleConnenection)
                ...
                If OleDatable.Rows.Count <> 0 Then
                    UpdateSociete = "Update COECRITURE SET Type='" & Trim(DateFin.Text) & "',Valeur='" & Trim(Txtintragroupe.Text) & "' where Libelle='" & Trim(Periode.Text) & "' Or Colonne='" & Trim(DateDebut.Text) & "'"
    ...
    Private Sub Creationperiode()
        ...
        Try
            If Trim(DateDebut.Text) <> "" And Trim(Periode.Text) <> "" And Trim(DateFin.Text) <> "" Then
                OleAdaptaterEnreg = New OleDbDataAdapter("select * From COECRITURE where Libelle='" & Trim(Periode.Text) & "' Or Colonne='" & Trim(DateDebut.Text) & "'", OleConnenection)
                ...
                Else
                    Insertion = "Insert Into COECRITURE (Colonne,Libelle,Libre,Type,Valeur) VALUES ('" & Join(Split(Join(Split(Join(Split(Join(Split(Trim(DateDebut.Text), " "), "_"), "-"), ""), "/"), ""), "\"), "") & "','" & Trim(Periode.Text) & "','1','" & Trim(DateFin.Text) & "','" & Trim(Txtintragroupe.Text) & "')"
    ...
    Private Sub supprimeperiode()
        ...
        Try
            If Trim(DateDebut.Text) <> "" And Trim(Periode.Text) <> "" And Trim(DateFin.Text) <> "" Then
                OleAdaptaterDelete = New OleDbDataAdapter("select * From COECRITURE where Libelle='" & Trim(Periode.Text) & "' Or Colonne='" & Trim(DateDebut.Text) & "'", OleConnenection)
                ...
                If OledatableDelete.Rows.Count <> 0 Then
                    DelFile = "Delete From COECRITURE where Libelle='" & Trim(Periode.Text) & "' Or Colonne='" & Trim(DateDebut.Text) & "'"
    ...
    Private Sub Initialiser()
        Try
            OleSocieteAdaptater = New OleDbDataAdapter("select * From COECRITURE where Libre='1' ", OleConnenection)
